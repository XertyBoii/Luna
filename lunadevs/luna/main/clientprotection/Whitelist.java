...
/*	
	  public static String HWID()
			    throws Exception
			  {
			    String hwid = HashHWID(System.getenv("PROCESSOR_IDENTIFIER"));
	  ...
	  private static String HashHWID(String text)
			    throws NoSuchAlgorithmException, UnsupportedEncodingException
			  {
			    ...
			    sha1hash = md.digest();
			    return HWIDData(sha1hash);
			  }
	  
	  private static String HWIDData(byte[] data)
	  {
	...
	public static void whitelist(){
		...
		try
		{
	      ...
	      if (!lines.contains(HWID()))
	      {
	        JOptionPane.showMessageDialog(null, "Your HWID is not whitelisted!");
	...
	*/
	...
	public static void whitelist(){
		...
		try
		{
		  ...
		  URL url = new URL(""); /** zCore/Main now controls HWID.*/
	      ArrayList<Object> lines = new ArrayList();
	      URLConnection connection = url.openConnection();
	      BufferedReader in = new BufferedReader(new InputStreamReader(connection.getInputStream()));
	      String line;
	      ...
	      if (!lines.contains(hwid.toString()))
	      {
	        JOptionPane.showMessageDialog(null, "Your HWID is not whitelisted! You cannot run the client.");
	        JOptionPane.showMessageDialog(null, "HWID Copied to clipboard!");
		    Toolkit.getDefaultToolkit().getSystemClipboard().setContents(new StringSelection(hwid.toString()), null);
	        System.exit(1);
	      }
		}
	    catch (Exception e)
