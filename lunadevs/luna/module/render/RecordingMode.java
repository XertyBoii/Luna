...
public class RecordingMode extends Module{
	...
	public RecordingMode() {
		super("Streaming", Keyboard.KEY_NONE, Category.RENDER, false);
	}
            ...
            if (ent != mc.thePlayer)
            {
                if (((ent instanceof EntityPlayer)) && (!ent.isInvisible()))
                {
                    ...
                    if (FriendManager.isFriend(ent.getName())) {
                        str = str.replace("§kAIDS", "\247b" + "§kAIDS");
                    }
	...
	public void onUpdate() {
		if (!this.isEnabled) return;
		if(ModuleManager.findMod(NewNameTags.class).isEnabled || ModuleManager.findMod(OldNameTags.class).isEnabled){
			ModuleManager.findMod(NewNameTags.class).setEnabled(false);
			ModuleManager.findMod(OldNameTags.class).setEnabled(false);
			
		}
		Entity entity;
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
        ...
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
            if (!(entity instanceof EntityCritFX)) continue;
            Minecraft.theWorld.removeEntity(entity);
        }
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
            if (!(entity instanceof EntityLargeExplodeFX)) continue;
            Minecraft.theWorld.removeEntity(entity);
        }
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
            if (!(entity instanceof EntityExplodeFX)) continue;
            Minecraft.theWorld.removeEntity(entity);
        }
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
            if (!(entity instanceof EntityHugeExplodeFX)) continue;
            Minecraft.theWorld.removeEntity(entity);
        }
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
            if (!(entity instanceof EntityEgg)) continue;
            Minecraft.theWorld.removeEntity(entity);
        }
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
            if (!(entity instanceof EntityPickupFX)) continue;
            Minecraft.theWorld.removeEntity(entity);
        }
        for (Object o2222222222 : Minecraft.theWorld.loadedEntityList) {
            entity = (Entity)o2222222222;
            if (!(entity instanceof EntityDropParticleFX)) continue;
            Minecraft.theWorld.removeEntity(entity);
            super.onUpdate();
        }
    }
	...
	public void onEnable() {
		...
		ModuleManager.findMod(NewNameTags.class).setEnabled(false);
		ModuleManager.findMod(OldNameTags.class).setEnabled(false);
		Parallaxa.addChatMessage("§cWARNING: §7Item Drop(s) won't be shown due to §7causing §7Lag.");
		Parallaxa.addChatMessage("Also boosts your FPS, TIP: Minimize Chat Scale.");
		super.onEnable();
	}
