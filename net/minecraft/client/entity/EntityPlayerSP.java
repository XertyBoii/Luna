...
import lunadevs.luna.events.EventOnLivingUpdate;
import lunadevs.luna.events.EventPlayerUpdate;
import lunadevs.luna.events.EventPostMotionUpdates;
import lunadevs.luna.events.EventSwing;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    {
    	EventManager.call(new EventPlayerUpdate(EventType.PRE));
        ...
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            {
            	PreMotionUpdatesEvent eventpre = new PreMotionUpdatesEvent(this.rotationYaw, this.prevRotationPitch, this.posY);
                ...
                EventManager.call(eventPostMotionUpdates);
                
            }
        }
        EventManager.call(new EventPlayerUpdate(EventType.POST));
    }

    public void func_175161_p()
    {
    	EventMotion event = new EventMotion(getLocation(), this.onGround, this.rotationYaw, this.rotationPitch, EventType.PRE);
    	EventManager.call(event);
    	
    	   TickPreEvent pre = new TickPreEvent(this.posX, getEntityBoundingBox().minY, this.posY, this.posZ);
        ...
        PostTickEvent posttick = new PostTickEvent();
        EventManager.call(posttick);
        ...
        EventManager.call(new EventMotion(getLocation(), this.onGround, this.rotationYaw, this.rotationPitch, EventType.POST));
    }
    ...
    {
      EventSwing event = new EventSwing();
      EventManager.call(event);
    ...
    public void setSprinting(boolean sprinting)
    {
        ...
        this.sprintingTicksLeft = sprinting ? 900 : 0;
    }
    ...
    public static boolean isInLiquid()
    {
      ...
      for (int x = MathHelper.floor_double(Minecraft.thePlayer.boundingBox.minX); x < 
            MathHelper.floor_double(Minecraft.thePlayer.boundingBox.maxX) + 1; x++) {
        for (int z = MathHelper.floor_double(Minecraft.thePlayer.boundingBox.minZ); z < 
              MathHelper.floor_double(Minecraft.thePlayer.boundingBox.maxZ) + 1; z++)
        {
    ...
    {
      for (int x = MathHelper.floor_double(this.boundingBox.minX); x < 
            MathHelper.floor_double(this.boundingBox.maxX) + 1; x++) {
        for (int y = MathHelper.floor_double(this.boundingBox.minY); y < 
              MathHelper.floor_double(this.boundingBox.maxY) + 1; y++) {
          for (int z = MathHelper.floor_double(this.boundingBox.minZ); z < 
                MathHelper.floor_double(this.boundingBox.maxZ) + 1; z++)
          {
            Block block = Minecraft.theWorld.getBlockState(
              new BlockPos(x, y, z)).getBlock();
            if ((block != null) && (!(block instanceof BlockAir)))
            {
              AxisAlignedBB boundingBox = block
                .getCollisionBoundingBox(Minecraft.theWorld, 
                new BlockPos(x, y, z), Minecraft.theWorld
    ...
    public float getDirection()
	  {
	    float yaw = this.rotationYawHead;float forward = this.moveForward;float strafe = this.moveStrafing;
    ...
    public double getSpeed() {
		return Math.sqrt(MathUtils.square(this.motionX) + MathUtils.square(this.motionZ));
	}
    ...
    {
        return Lists.newArrayList(new ItemStack[] {this.getHeldItem(), this.getHeldItem()});
    }
}
